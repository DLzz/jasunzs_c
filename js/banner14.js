...
$(document).ready(function ()
{
    ...
    {
        $.ajax({ url: "http://1.rc.xiniu.com/js/xiniunew/banner.txt", data: { key1: 1, key2: 2 }, success: initSuccess, error: initError, dataType: "text" });
    }
...
var xn_ba_js_14_start = function ()
{
    ...
    //初始化加载第一张图片2016-02-27
    try
    {
        var firstImg = $(".xn_ba_js_14_bigImg").find("img").eq(0);
        var firstUrl = firstImg.attr("data-original");
        if (firstUrl)
        {
            $(".xn_ba_js_14_bigImg").find("img").eq(0).attr("src", firstUrl);
        }
    } catch (e)
    {
    ...
    function skipHandler()
    {
        ...
        try
        {
            var thisImg = indexPic.find("img").eq(0);
            var imgNewSrc = thisImg.attr("data-original");
            var imgOldSrc = thisImg.attr("src");
            if (imgNewSrc != imgOldSrc)
            {
                thisImg.attr("src", imgNewSrc);
            }
        }
        catch (e)
        {
